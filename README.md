...
TextCleaner is a workspace for modernizing and maintaining Word add-ins and related tooling.
...
3. Run the dev build or start the dev server (see package.json scripts):
...
- Do not commit `.env` or other secret-containing files. Use `.env.example` as a template.
- This repo was initialized locally; push to GitHub with `git push -u origin main` after creating a remote.

See the `word-addin-modernization/` folder for migration notes and planning.
